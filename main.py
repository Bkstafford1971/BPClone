...
from ai           import (
    get_or_create_rivals, save_rivals, assign_ai_gear,
    assign_ai_strategies, assign_ai_training, rival_summary, RivalManager,
)
...
class GameState:
    """Holds the active player team and rival pool for the session."""
...
def main_menu(gs: GameState):
    while True:
        ...
        elif MAIN_MENU_OPTIONS[idx] == "View rival pool":
            _view_rivals(gs)
...
def _view_rivals(gs: GameState):
    header("Rival Pool")
    if not gs.rivals:
        gs.rivals = get_or_create_rivals()
    print(rival_summary(gs.rivals))
    pause()
