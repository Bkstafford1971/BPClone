...
def _check_league_reset() -> dict:
    ...
    league_url = settings.get("server_url", "").rstrip("/")
    if not league_url:
...
def _acknowledge_league_reset(body: dict) -> dict:
    ...
    Resets local arena state (clears records, keeps rosters) and
    stores the new reset_count so the prompt won't reappear.
    """
    from save import reset_arena_state
    result = reset_arena_state()
    ...
    return {"success": True, "message": "Local records cleared. Ready for the new season."}
